subroutine initialize
  ...
  frest     = 'out_'//TRIM(problemname)//'.rest'
  fimar     = 'out_'//TRIM(problemname)//'.r'
  ...
  if(sidt.gt.0)then
     ...
     allocate(part(numpart)) ! allocate particle structure if the restart option is not activated
     !
     ...
     do iphase=1,nphases 
        ...
        if(phase(iphase)%aggregation)then
           ...
           do ibin=1,ibin2
             ...
             npbin(ibin)=int(a*(r**(phase(iphase)%bins-ibin))) !no sumar uno
