...
MODULE InpOut
  ...
  character(len=s_name) :: frest            ! Output file: restart
  character(len=s_name) :: fimar            ! Output file: Script to execute in R  
  ...
  !
  !
  real (rp), allocatable:: conc2d(:)         !ielement
  real (rp), allocatable:: conc3d(:)
  real (rp), allocatable:: sedim (:)
  !
  ...
  !*** Other flags
  !
  logical :: out_screen   = .true.          !  If out_screen = .true. prints processes on screen (not only on log file)
  !
  !*** List of Warnings 
  !
