  ...
  !
  real   (rp)              :: rvoid(1)
  real   (rp)              :: sit           ! Simulation duration time
  real   (rp)              :: work(2)
  integer(ip)              :: i,iz,nz
  ...
  if(sidt.gt.0) then
     ...
     call addtime(ibyr, ibmo, ibdy, hrst, iyr, imo, idy, ihr, imi, ise, sit)
     time_end   = 1d8*iyr + 1d6*imo + 1d4*idy + 1d2*ihr + imi
  else if (sidt.lt.0) then
     time_start = 1d8*ibyr+1d6*ibmo+1d4*ibdy+1d2*hrst+mist 
     sit        = -siend+sist- mist-sest            ! Total time in seconds beginning in hrst
     call subtime(ibyr, ibmo, ibdy, hrst, iyr, imo, idy, ihr, imi, ise, sit)
     time_end   = 1d8*iyr + 1d6*imo + 1d4*idy + 1d2*ihr + imi  
  end if
  !  
           ...
           do while(go_on)                  !Read the path for the corresponding file
              ...
              do while(0.eq.0)
                 ...
                 if(TRIM(words(1)).eq.'PHASE_NAME') then
                    ...
                    write(lulog,4) nphases, phase(nphases)%npart
4                   format(/, 'The total number of particles to simulate in phase	', i2, '	is     : ', i10)
                    ...
                    !
                    if(phase(nphases)%source_type.eq.'RESUSPENSION') phase(nphases)%aggregation=.false.  ! Switch-off
                                                                                   ! aggregation in resuspension mode
