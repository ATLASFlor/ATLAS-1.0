  ...
  !*    
  !*    Serial Version.
  !*    Author: Florencia Reckziegel & Arnau Folch
...
!
     ...
     !
     !*** Write actual time in log file
     !
!     if(out_screen) &
!     write(*    ,4) actualtime
     write(lulog,4) actualtime 
4    format(/, 'Current time     : ', a)
     !  
        ...
        if(phase(iphase)%activated)then
           ...
           do ipart = phase(iphase)%firstpart,npart2
              ...
              if(part(ipart)%state==1)then 
                 ...
                 !wold=part(ipart)%wa !wold is positive down
                 ! 
                 ...
                 call diffusion(part(ipart)%ua,part(ipart)%va,part(ipart)%wa,part(ipart)%z, &
                                ...
                                part(ipart)%uvw(1),part(ipart)%uvw(2),part(ipart)%uvw(3))
                 end if
                 ! 
                 ...
                 !    180 to 180 longitude-------------->PBC_EW=true, change longitude
                 !    180 to 180 long and latitude 90--->PBC_N =true, change longitude and latitude
                 !    180 to 180 long and latitude -90-->PBC_S =true, change longitude and latitude
                 !
                 call checkdomain(part(ipart)%state,part(ipart)%lon,part(ipart)%lat,part(ipart)%z)
                 !
                 ! si resuspension = .true. que la particula no baje más->(?)
                 !
