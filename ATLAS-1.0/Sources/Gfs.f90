...
CONTAINS
  ...
  subroutine opengfs(nm)
    ...
    !
    write(lulog,2)  'Opened GFS file and readed necesary Time Independent variables. Created mesh structure.'
         ...
         ipoin = ipoin+1
             if(grid%model(ipoin).eq.nm)then
                ...
                call get_par_ABL(1.0_rp,grid%z(2),grid%T(ix,iy,1,2),grid%T(ix,iy,2,2),p(ix,iy,1),p(ix,iy,2), &
                   grid%u(ix,iy,1,2),grid%v(ix,iy,1,2),grid%ust(ix,iy,2),grid%rmonin(ix,iy,2),grid%hfx(ix,iy,2))
                 !
