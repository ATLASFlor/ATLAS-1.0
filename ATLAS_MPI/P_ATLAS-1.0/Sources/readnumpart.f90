subroutine readnumpart
  ...
  ! Determine the lastpart for restart phase and the number of active particles
         if(split)then
           phase(nphases)%totpart=phase(nphases)%npart*2**(INT((siend-sist)/dts))
           phase(nphases)%lastpart=phase(nphases)%firstpart+ phase(nphases)%totpart
         else
           phase(nphases)%lastpart=phase(nphases)%firstpart+ phase(nphases)%npart
           phase(nphases)%totpart=phase(nphases)%npart
         end if
           phase(nphases)%actnpart=phase(nphases)%totpart !npart
  !
