!***************************************************************
!*
!*              Module for ERA5 Reanalysis Meteo
!* 
!***************************************************************
MODULE ERA5
  use KindType
  ...
  !
  ! ERA5 DIMENSIONS:
  character(len=25) :: nx_era5_name      = 'lon'
  character(len=25) :: ny_era5_name      = 'lat'
  character(len=25) :: np_era5_name      = 'pres'
  character(len=25) :: nt_era5_name      = 'time'
  !
  !ERA5 VARIABLES:
  character(len=25) :: time_era5_name    = 'time'
  character(len=25) :: lon_era5_name     = 'lon'
  character(len=25) :: lat_era5_name     = 'lat'
  character(len=25) :: hgt_era5_name     = 'Z:sfc'
  character(len=25) :: u_era5_name       = 'U'
  character(len=25) :: v_era5_name       = 'V'
  character(len=25) :: w_era5_name       = 'W'
  character(len=25) :: T_era5_name       = 'T'
  character(len=25) :: p_era5_name       = 'pres'
  character(len=25) :: pblh_era5_name    = 'BLH:sfc'
  character(len=25) :: q_era5_name       = 'R'
  character(len=25) :: fi_era5_name      = 'Z'
  character(len=25) :: landmask_era5_name= 'LSM:sfc'

  character(len=25) :: us_era5_name    = 'U10:sfc'
  character(len=25) :: vs_era5_name    = 'V10:sfc'
  character(len=25) :: Ts_era5_name    = 'T2:sfc'
  !
  !ERA5 ATTRIBUTES:
  character(len=25) :: ibyr_era5_name = 'YEAR'
  character(len=25) :: ibmo_era5_name = 'MONTH'
  character(len=25) :: ibdy_era5_name = 'DAY'
  character(len=25) :: ibhr_era5_name = 'HOUR'
  character(len=25) :: dt_era5_name   = 'TIME_INCR'
  character(len=25) :: lonmin_era5_name = 'LONMIN'
  character(len=25) :: lonmax_era5_name = 'LONMAX'
  character(len=25) :: latmin_era5_name = 'LATMIN'
  character(len=25) :: latmax_era5_name = 'LATMAX'


  !
CONTAINS
  !
  subroutine openera5(nm)
    !******************************************************************************************
    !*    OPENERA5
 ...
 !   real     (rp)         :: tduration                    ! Total Time Duration.in seconds
    real     (rp)         :: lonmin_era5, lonmax_era5 ! Longitud max and min. Attribute
    real     (rp)         :: latmin_era5, latmax_era5 ! Latitud max and min. Attribute
 !   real     (rp)         :: dy_era5                    ! Y resolution
    ! Dimensions names
    ...
    !
    !
    real(rp), allocatable :: era5_coord(:,:)
    real(rp), allocatable :: work2d(:,:)            ! Local array used for topography
    real(rp), allocatable :: lwork2d(:,:)           ! Local array used for landmask
    real(rp), allocatable :: lon_era5(:)
    real(rp), allocatable :: lat_era5(:)
    !
    !*** ERA 5 Time
    !
    integer(ip)   :: yr0_era5,mo0_era5,dy0_era5,hr0_era5
    ...
    !
    !*** Open the Era 5 model. Gets the Era5 file ID 
    !
    if(NF90_OPEN(meteo(nm)%file_path,NF90_NOWRITE,ncID)/=0) &
         call runend('Error in  nf90_open Era 5 file') 
    !
    write(ext,'(i1)') nm
    str = 'METEO_DATA_'//ext
    !
    ...
    !
    if(nf90_inq_dimid(ncID, nx_era5_name, NXdimid)/=NF90_NOERR) &
         call runend('Error inquiring nx dimension id in Era 5 file') 
    if(nf90_inquire_dimension(ncID, NXdimid, NXname, meteo(nm)%nx)/=NF90_NOERR) &
         call runend('Error inquiring NX dimension in Era 5 file')
    !
    !*** Gets the Id number for the Ny Dimension.
    !
    if(nf90_inq_dimid(ncID, ny_era5_name, NYdimid)/=NF90_NOERR) &
         call runend('Error inquiring ny dimension id in era5 file') 
    if(nf90_inquire_dimension(ncID, NYdimid, NYname, meteo(nm)%ny)/=NF90_NOERR) &
         call runend('Error inquiring NY dimension in era5 file')
    !     
    !*** Gets the Id number for the NP number of pressure levels Dimension and read it. (np)
    !
    if(nf90_inq_dimid(ncID, np_era5_name, NPdimid)/=NF90_NOERR) &
         call runend('Error inquiring np dimension id in era5 file') 
    if(nf90_inquire_dimension(ncID, NPdimid, NPname, meteo(nm)%np)/=NF90_NOERR) &
         call runend('Error inquiring NP dimension in era5 file')
    !
    !*** Gets the ID number for Time dimension. Read nt
    ! 
    if(nf90_inq_dimid(ncID, nt_era5_name, Ntdimid)/=NF90_NOERR) &
         call runend('Error inquiring nt dimension id in era5 file') 
    if(nf90_inquire_dimension(ncID, Ntdimid, Ntname, meteo(nm)%nt)/=NF90_NOERR) &
         call runend('Error inquiring Nt dimension in era5 file')
    !
    !*** allocate memory
    !
    ...
    !
    allocate (lon_era5 (  meteo(nm)%nx             ))
    allocate (lat_era5 (  meteo(nm)%ny             ))
    ...
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,ibyr_era5_name, yr0_era5)/=NF90_NOERR)  &
         call runend('Error getting attribute YEAR in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,ibmo_era5_name, mo0_era5)/=NF90_NOERR) &
         call runend('Error getting attribute MONTH in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,ibdy_era5_name, dy0_era5)/=NF90_NOERR)   &
         call runend('Error getting attribute DAY in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,ibhr_era5_name, hr0_era5)/=NF90_NOERR)   &
         call runend('Error getting attribute DAY in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,dt_era5_name, tincr)/=NF90_NOERR)   &
         call runend('Error getting attribute DAY in openera5')
    !
    !*** Get lon lat min and max. attributes
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,lonmax_era5_name, lonmax_era5)/=NF90_NOERR) &
         call runend('Error getting attribute lonmax in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,lonmin_era5_name, lonmin_era5)/=NF90_NOERR) &
         call runend('Error getting attribute lonmin in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,latmax_era5_name, latmax_era5)/=NF90_NOERR) &
         call runend('Error getting attribute latmax in openera5')
    !
    if(nf90_get_att(ncID,NF90_GLOBAL,latmin_era5_name, latmin_era5)/=NF90_NOERR) &
         call runend('Error getting attribute latmin in openera5')
    !
    !*** Get time variable
    !
    if(nf90_inq_varid(ncID, time_era5_name, tvarID)/=NF90_NOERR) &
         call runend('Error getting Time variable id in openera5')
    if(nf90_get_var(ncID,tvarID,meteo(nm)%timesec)/=NF90_NOERR) &
         call runend('Error getting variable time in openera5')
    !
    !*** Calculate the day, month, year, hour, minute for each time step in format yyyymmddhhmm
    !
    allocate (meteo(nm)%time(meteo(nm)%nt))
    do it = 1, meteo(nm)%nt 
       call addtime(yr0_era5, mo0_era5, dy0_era5, hr0_era5, iyr, imo, idy, ihr, imi, ise,meteo(nm)%timesec(it))
       meteo(nm)%time(it) = 1d8*iyr + 1d6*imo + 1d4*idy + 1d2*ihr + imi
    end do
    !
    !***  Calculates time lag by iteration (that is, the time in seconds between the Era 5 origin
    ...
    do while(.not.found)
       call addtime(yr0_era5,mo0_era5,dy0_era5,0, &
    ...
    !
    !*** Get ERA 5 Lon and Lat variables 
    !
    if(nf90_inq_varid(ncID, lon_era5_name, lonvarID)/=NF90_NOERR) &
         call runend('Error getting lon variable id in openera5')
    if(nf90_get_var(ncID,lonvarID,lon_era5)/=NF90_NOERR) &
         call runend('Error getting variable lon in openera5')
    !
    if(nf90_inq_varid(ncID, lat_era5_name, latvarID)/=NF90_NOERR) &
         call runend('Error getting lat variable id in openera5')
    if(nf90_get_var(ncID,latvarID,lat_era5)/=NF90_NOERR) &
         call runend('Error getting variable lat in openera5')
    !
    !*** Get Era 5 topography at box center 'Z:sfc' variable in Era5
    !
    if(nf90_inq_varid(ncID, hgt_era5_name, hgtvarID)/=NF90_NOERR) &
         call runend('Error getting hgt variable id in openera5')
    if(nf90_get_var(ncID,hgtvarID,work2d)/=NF90_NOERR) &
         call runend('Error getting variable hgt in openera5')
    work2d = work2d / 9.81_rp            ! topg = (fis)/g
    !
    !*** Get Era5 landmask at box center 
    !
    if(nf90_inq_varid(ncID, landmask_era5_name, landmaskvarID)/=NF90_NOERR) &
         call runend('Error getting landmask variable id in openera5')
    if(nf90_get_var(ncID,landmaskvarID,lwork2d)/=NF90_NOERR) &
         call runend('Error getting variable landmask in openera5')
    work2d=work2d*lwork2d
    !
    !*** Calculate the model resolution. In degrees
    !
    meteo(nm)%res=(latmax_era5-latmin_era5)/(meteo(nm)%ny-1)  
    !
    !*** Determine if the meteorological model is global (horizontally)
    !
    if(lon_era5(meteo(nm)%nx)+meteo(nm)%res.eq.180 .and. lon_era5(1).eq.-180) meteo(nm)%global=.true.
    !
    !
    ...
    if(meteo(nm)%global.eqv..false.)then
       ...
       ! Local arrays
       allocate (era5_coord(2,meteo(nm)%npoin2d   ))
       !
       ...
       do iy=1,meteo(nm)%ny
          do ix=1,meteo(nm)%nx
             meteo(nm)%lon(ix,iy)=lon_era5(ix)
             meteo(nm)%lat(ix,iy)=lat_era5(iy)
          end do
       end do
       ...
       do iy = 1,meteo(nm)%ny
          do ix = 1,meteo(nm)%nx
             ipoin = ipoin + 1
             era5_coord(1,ipoin) = meteo(nm)%lon(ix,iy)
             era5_coord(2,ipoin) = meteo(nm)%lat(ix,iy)
          end do
       end do
       !
       !
       ...
       do iy=1,meteo(nm)%ny
          ...
          meteo(nm)%lat(meteo(nm)%nx+1,iy)=lat_era5(iy)
          do ix=1,meteo(nm)%nx
             meteo(nm)%lon(ix,iy)=lon_era5(ix)
             meteo(nm)%lat(ix,iy)=lat_era5(iy)
          end do
    ...
    do ipoin = 1,grid%npoin2d   ! loop over background mesh points
       ...
       call elsest(&
            ...
            meteo(nm)%lnods,ltopo,era5_coord,xcoor,relse,&
    ...
    !
    if(my_id.eq.0)write(lulog,2)  'Opened ERA 5 file and readed necesary Time Independent variables. Created mesh structure.'
    if(out_screen) write(*,2) 'Opened ERA 5 file and readed necesary Time Independent variables. Created mesh structure.'
    !
2   format(/,  a /)
    ! 
    ...
    return
    !
  end subroutine openera5
  ! 
  ! 
  ! 
  subroutine readera5(nm,itime)
    !******************************************************************************************
    !*    READERA5
    ...
    if(NF90_OPEN(meteo(nm)%file_path,NF90_NOWRITE,ncID)/=NF90_NOERR) &
         call runend('Error in  nf90_open era5 file') 
    !
    !*** Gets Variables ID
    !
    if( nf90_inq_varid(ncID,fi_era5_name,fiID) /= NF90_NOERR) &
         call runend('Error getting fiID in '//TRIM(str))
    !
    if( nf90_inq_varid(ncID,u_era5_name,uID) /= NF90_NOERR) &
         call runend('Error getting uID in '//TRIM(str))
    !
    if( nf90_inq_varid(ncID,v_era5_name,vID) /= NF90_NOERR) &
         call runend('Error getting vID in '//TRIM(str))
    !
    if( nf90_inq_varid(ncID,w_era5_name,wID) /= NF90_NOERR) &
    ...
    if( nf90_get_var(ncID,uID,work_ERA1,start=(/1,1,1,itime/)) /=NF90_NOERR ) &
         call runend('read_era5: Error reading u in '//TRIM(str))
    !
    do iz=1,meteo(nm)%np
       ipoin=0
       do iy=1,meteo(nm)%ny
    ...
    if( nf90_get_var(ncID,vID,work_ERA1,start=(/1,1,1,itime/)) /=NF90_NOERR ) &
         call runend('read_era5: Error reading v in '//TRIM(str))
    !
    do iz=1,meteo(nm)%np
       ipoin=0
       do iy=1,meteo(nm)%ny
    ...
    !
    !*** Reads p levels for Era5 mesh
    !
    allocate (p_ERA (meteo(nm)%np))
    allocate (p     (meteo(nm)%nx,meteo(nm)%ny,meteo(nm)%np))
    if( nf90_inq_varid(ncID,p_era5_name,pID) /= 0) call runend('read_era5 : Error getting pID')
    if( nf90_get_var  (ncID,pid,p_ERA) /= 0 ) call runend('read_era5 : Error in nf90_get_var')
    ...
    if( nf90_get_var(ncID,wID,work_ERA1,start=(/1,1,1,itime/)) /=NF90_NOERR ) &
         call runend('read_era5: Error reading w in '//TRIM(str))
    !
    do iz=1,meteo(nm)%np
       ipoin=0
       do iy=1,meteo(nm)%ny
    ...
    !
    if( nf90_inq_varid(ncID,T_era5_name,tID) /= 0) call runend('read_era5 : Error getting tID')
    if( nf90_get_var(ncID,tID,work_ERA1,start=(/1,1,1,itime/)) /=0 ) call runend('read_era5: Error reading T')
    !
    do iz=1,meteo(nm)%np
       ipoin=0
       do iy=1,meteo(nm)%ny
    ...
    !
    !*** RH(nx_era5,ny_era,nz_era5) Relative humidity --> qv(nx,ny,nz)  Specific humidity is computed later
    !
    if( nf90_inq_varid(ncID,Q_era5_name,qID) /= 0) call runend('read_era5 : Error getting qID')
    if( nf90_get_var(ncID,qID,work_ERA1,start=(/1,1,1,itime/)) /=0 ) call runend('read_era5: Error reading QV')
    !
    do iz=1,meteo(nm)%np
       ipoin=0
       do iy=1,meteo(nm)%ny
    ...
    !
    !*** pblh(nx_era5,ny_era5) --> pblh(nx,ny) Atmospheric Boundary Layer
    !
    if( nf90_inq_varid(ncID,pblh_era5_name,pblhID) /= 0) call runend('read_era5 : Error getting pblhID')
    if( nf90_get_var(ncID,pblhID,work_ERA2,start=(/1,1,itime/)) /=0 ) call runend('read_era5: Error reading hpbl')
    !
       ipoin=0
       do iy=1,meteo(nm)%ny
          do ix=1,meteo(nm)%nx
    ...
    !
    !
    !*** u:sfc(nx_era5,ny_era5)
    !
    if( nf90_inq_varid(ncID,us_era5_name,usfcID) /= 0) call runend('read_era5 : Error getting usfcID')
    if( nf90_get_var(ncID,usfcID,work_ERA2,start=(/1,1,itime/)) /=0 ) call runend('read_era5: Error reading u:sfc')
    !
       ipoin=0
       do iy=1,meteo(nm)%ny
          do ix=1,meteo(nm)%nx
    ...
    !
    !*** v:sfc(nx_era5,ny_era5)
    !
    if( nf90_inq_varid(ncID,vs_era5_name,vsfcID) /= 0) call runend('read_era5 : Error getting vsfcID')
    if( nf90_get_var(ncID,vsfcID,work_ERA2,start=(/1,1,itime/)) /=0 ) call runend('read_era5: Error reading v:sfc')
    !
       ipoin=0
       do iy=1,meteo(nm)%ny
          do ix=1,meteo(nm)%nx
    ...
    !
    !*** T:sfc(nx_era5,ny_era5)
    !
    if( nf90_inq_varid(ncID,ts_era5_name,tsfcID) /= 0) call runend('read_era5 : Error getting TsfcID')
    if( nf90_get_var(ncID,tsfcID,work_ERA2,start=(/1,1,itime/)) /=0 ) call runend('read_era5: Error reading T:sfc')
    !
       ipoin=0
       do iy=1,meteo(nm)%ny
          do ix=1,meteo(nm)%nx
    ...
    deallocate(work_ERA )
    deallocate(p_ERA)
    if(NF90_close(ncID)/=NF90_NOERR) call runend('Error in  nf90_close era5 file') 
         ...
         ipoin = ipoin+1
             if(grid%model(ipoin).eq.nm)then
                ...
                call get_par_ABL(1.0_rp,grid%z(2),wgridtsfc(ix,iy),grid%T(ix,iy,2,2),p(ix,iy,1),p(ix,iy,2), &
                   wgridusfc(ix,iy),wgridvsfc(ix,iy),grid%ust(ix,iy,2),grid%rmonin(ix,iy,2),grid%hfx(ix,iy,2))
                 !
